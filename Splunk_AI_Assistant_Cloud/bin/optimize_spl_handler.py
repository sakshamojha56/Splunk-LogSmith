...
class OneshotOptimizeSPLHandler(PersistentServerConnectionApplication, BaseRestUtils):
    ...
    def handle_oneshot_optimizespl_generation(
        self,
        saia_api,
        params,
        job_id,
        ...
        if self.V2_FLAG:
            ...
            result = saia_api.spl_optimize(**request_kwargs)
            result_body = result.json()
            optimized_spl = result_body["spl"]
        else:
            request_kwargs = dict(
                job_id=job_id,
                user_prompt=user_prompt,
        ...
        return self.create_response(json.dumps({"response": optimized_spl}), 200)
    ...
    def handle_func(self, request):
        ...
        should_log_telemetry, _ = self._fetch_telemetry_details(
            service, request, "oneshot-optimize-spl"
        )

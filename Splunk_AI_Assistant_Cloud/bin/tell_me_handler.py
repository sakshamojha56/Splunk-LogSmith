...
class OneshotTellMeHandler(PersistentServerConnectionApplication, BaseRestUtils):
    ...
    def handle_oneshot_tellme_generation(
        self,
        saia_api,
        params,
        job_id,
        ...
        if self.V2_FLAG:
            ...
            result = saia_api.ask_splunk_question(**request_kwargs)
            result_body = result.json()
            response_text = result_body["answer"]
        else:
            request_kwargs = dict(
                job_id=job_id,
                user_prompt=user_prompt,
        ...
        try:
            ...
            if source_urls and source_titles and len(source_urls) == len(source_titles):
                resource_links = [
                    {"href": u, "description": t}
                    for u, t in zip(source_urls, source_titles)
                ]
        except Exception:
        ...
        return self.create_response(
            json.dumps({"response": response_text, "resourceLinks": resource_links}),
            200,
        )
    ...
    def handle_func(self, request):
        ...
        should_log_telemetry, _ = self._fetch_telemetry_details(
            service, request, "oneshot-tell-me"
        )

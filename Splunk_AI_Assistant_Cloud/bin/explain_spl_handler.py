...
class OneshotExplainSPLHandler(PersistentServerConnectionApplication, BaseRestUtils):
    ...
    def handle_oneshot_explainspl_generation(
        self,
        saia_api,
        params,
        job_id,
        ...
        if self.V2_FLAG:
            user_prompt = (
                f"Explain this SPL query in detail:\n\n```spl\n{params['spl']}\n```"
            )
            parsed_payload = self.parse_mcp_request_payload(
        ...
        if self.V2_FLAG:
            ...
            result = saia_api.spl_explain(**request_kwargs)
            result_body = result.json()
            response_text = result_body["explanation"]
        else:
            request_kwargs = dict(
                job_id=job_id,
                user_prompt=user_prompt,
        ...
        return self.create_response(json.dumps({"response": response_text}), 200)
    ...
    def handle_func(self, request):
        ...
        should_log_telemetry, _ = self._fetch_telemetry_details(
            service, request, "oneshot-explain-spl"
        )

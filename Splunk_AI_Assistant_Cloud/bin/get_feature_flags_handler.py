...
Persistent REST handler that serves `GET /get_feature_flags` for the SAIA Splunk app.

- Response schema: JSON object with `enabled_feature_flags` (list of flag ids) representing the
  currently enabled feature flags as stored locally, and `permissions` describing allowed indexes,
  sourcetypes, and roles for the current user.
"""
...
class FeatureFlagsHandler(PersistentServerConnectionApplication, BaseRestUtils):
    ...
    def handle_func(self, request):
        ...
        - Returns enabled feature flags and the current user's permissions object.
        """
        ...
        self.logger.info(
            log_kwargs(
                message="Handling feature flag fetch request.",
                **logging_context,
            )
        )

        metadata_collection = MetadataCollection(service)
        # Feature flag synchronization still uses the v1 metadata APIs, so keep v1 hardcoded.
        api_client = SaiaApi(
        ...
        feature_settings = metadata_entry.get(
            MetadataCollection.KEY_FEATURE_SETTINGS, {}
        )
        feature_settings_header = api_client._construct_feature_flags_header(
            feature_settings
        )

        response_payload = {
            "feature_settings_header": feature_settings_header,
        }

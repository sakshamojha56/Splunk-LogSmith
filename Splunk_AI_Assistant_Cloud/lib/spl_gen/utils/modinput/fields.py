...
class BooleanField(Field):
    def to_python(self, value):
        ...
        raise FieldValidationException(
            f"The value of '{str(value)}' for the '{self.name}' parameter is not a valid boolean"
        )
...
class DelimitedField(Field):
    def __init__(self, name, title, description, delim, required_on_create=True, required_on_edit=False):
        super().__init__(name, title, description, required_on_create, required_on_edit)
        self._delim = delim
...
class DurationField(Field):
    ...
    def to_python(self, value):
        ...
        if m is None:
            raise FieldValidationException(
                f"The value of '{str(value)}' for the '{self.name}' parameter is not a valid duration"
            )
        ...
        try:
            ...
            raise FieldValidationException(
                f"The duration '{d['duration']}' for the '{self.name}' parameter is not a valid number"
            )

        # Make sure the units are valid
        if len(units) > 0 and units not in DurationField.UNITS:
            raise FieldValidationException(
                f"The unit '{units}' for the '{self.name}' parameter is not a valid unit of duration"
            )
...
class IntervalField(Field):
    ...
    def to_python(self, value):
        ...
        raise FieldValidationException(f"The value of '{value}' for the '{self.name}' parameter is not a valid value")
...
class JsonField(Field):
    def to_python(self, value):
        ...
        try:
            ...
            raise FieldValidationException(
                f"The value of '{str(value)}' for the '{self.name}' parameter is not a valid JSON object"
            )

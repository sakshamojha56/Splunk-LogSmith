...
class KVStoreLookupProcessor(BaseProcessor):
        ...
        try:
            ...
            if reply['success']:
                for i, json_content_row in enumerate(json_content):
                    ...
                    for key, value in json_content_row.items():
                        if isinstance(value, list):
                            value = list(map(cls.json_dumps_unless_string, value))

                            # if the value is a list, it needs to be multivalue encoded
                            json_content[i][key] = '\n'.join(value)
                            encoded_multivalues['__mv_' + key] = ';'.join(
                                map(multivalue_encode, value)
                            )
                        else:
    ...
    def load_collection(cls, collection_name, searchinfo):
        ...
        kvstore_user_reply = kvstore_util.load_collection_from_rest(
            'user', collection_name, rest_proxy
        )
        kvstore_shared_reply = kvstore_util.load_collection_from_rest(
            'app', collection_name, rest_proxy
        )

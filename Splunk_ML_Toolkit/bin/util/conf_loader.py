...
class RestLoadingStrategy(object):
    ...
    def load_container_conf(self, stanza_name=None):
        """
        Load the conf file (all stanzas or a single stanza) from the REST endpoint.

        Args:
            stanza_name (str, optional): Name of stanza to load. If None, loads all stanzas.

        Returns:
            resp (dict): response from endpoint
        """
        if stanza_name:
            # Load only the given stanza
            url = '{uri}/{namespace}/nobody/{app}/configs/conf-{conf_name}/{stanza}?output_mode=json&count=-1'.format(
                uri=self.proxy.splunkd_uri,
                namespace=self.proxy.name_space_str,
                app=self.proxy.splunk_app,
                conf_name=self.conf_name,

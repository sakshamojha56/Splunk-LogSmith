...
def create_output_dataframe(y_hat, nans, output_names, shape=None):
    ...
    # Python 3.13 compatibility: Improved dtype handling for pandas compatibility
    if len(y_hat) > 0:
        # Quick dtype check without creating copies - only check the first element
        try:
            # Use a more efficient approach to check data type
            if hasattr(y_hat, 'dtype'):
                # If y_hat is an object array but with mixed types,
                # let pandas handle dtype inference per column
                if y_hat.dtype.kind == 'O' and y_hat.ndim > 1:
                    # Multi-dimensional object array - let pandas infer column dtypes
                    pass
                elif y_hat.dtype.kind in ['U', 'S', 'O']:
                    # String or single-type object array
                    output = output.astype(object)
            else:
                # For list-like objects, check first element only
                sample_val = (
                    y_hat[0] if hasattr(y_hat, '__getitem__') and len(y_hat) > 0 else y_hat
                )
                need_object_dtype = isinstance(sample_val, (str, bytes))
                if need_object_dtype:
                    output = output.astype(object)
        except (IndexError, TypeError):
            # Fallback for edge cases - assume numeric
            pass

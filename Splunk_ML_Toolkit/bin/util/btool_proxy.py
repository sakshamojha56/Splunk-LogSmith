...
class BtoolProxy(object):
    ...
    def get_container_stanza_mapping(self, conf_name):
        """Get stanza mapping for MLSPL Conf settings.
        ...
        settings = {}
        for user_or_role in self.users_and_roles:
            container_settings = get_container_btool(
                user_or_role, self.app, self.target_dir, conf_name
            )
            settings.update(container_settings)

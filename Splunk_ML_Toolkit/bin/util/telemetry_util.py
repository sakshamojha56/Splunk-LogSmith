...
example_names = {
    ...
    'cdtsm_example_I': 'Forecasting',
    'cdtsm_example_II': 'Anomaly Detection',
}
...
algorithm_and_parameter_white_list = {
    ...
    'AITKContainer': {},
    'container': {},
    'sagemaker_custom_model': {'runtime', 'features'},
    'vertex_custom_model': {'runtime', 'features'},
}
...
@safe_func
def log_hpa_usage(enabled):
    """
    Log whether Horizontal Pod Autoscaler is enabled (1) or disabled (0).
    """
    logger.debug("hpa_enabled=%d" % (1 if bool(enabled) else 0))
...
def log_apply_container_time(
    ...
    logger.debug(
        "command=apply, apply_time=%f, hpa_enabled=%d, algo_name=%s, container_id=%s, "
        "cluster_type=%s, auth_mode=%s, min_replicas=%s, max_replicas=%s, "
        "min_cpu=%s, max_cpu=%s, min_memory=%s, max_memory=%s, csv_size=%s, model_name=%s"
        % (
            interval,
            hpa_enabled,
            algo_name,
...
@safe_func
def log_kb_usage(kb_used):
    logger.debug("command=ai, kb_used=%d" % (kb_used))
    return 1 if kb_used else 0
...
def log_fit_container_time(
    ...
    logger.debug(
        "command=fit, fit_time=%f, hpa_enabled=%d, algo_name=%s, container_id=%s, "
        "cluster_type=%s, auth_mode=%s, min_replicas=%s, max_replicas=%s, "
        "min_cpu=%s, max_cpu=%s, min_memory=%s, max_memory=%s, csv_size=%s, model_name=%s"
        % (
            interval,
...
@safe_func
def log_container_start(container_id, status):
    logger.debug(
        "command=container, container_id=%s, container_status=%s" % (container_id, status)
    )


@safe_func
def log_container_stop(container_id, status):
    logger.debug(
        "command=container, container_id=%s, container_status=%s" % (container_id, status)
    )

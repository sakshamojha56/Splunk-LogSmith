# !/usr/bin/env python
# Python 3.13 compatibility: Fixed convert_df_to_categorical return tuple and enhanced categorical conversion for sklearn compatibility
import importlib
...
def convert_df_to_true_binary(df, pos_label):
    ...
    Returns:
        tuple: (df, converted_pos_label) where:
            df (pd.dataframe): original df with data contained in
                fields converted to true-binary inplace
            converted_pos_label (int): pos_label converted to match the data dtype
    """
    ...
    # Ensure all columns have proper numeric dtype for sklearn compatibility
    # Convert all columns to proper integer dtype to avoid sklearn 'unknown' type_of_target
    for col in df.columns:
        try:
            # Convert to numeric first, then to int, and replace the column entirely
            numeric_values = pd.to_numeric(df[col], errors='raise')
            df[col] = numeric_values.astype('int64')
        except (ValueError, TypeError):
            # If conversion fails, keep as is
            pass

    # Convert pos_label to match the data dtype (integer)
    # After conversion, the data will be integers (0, 1), so pos_label should be integer too
    converted_pos_label = pos_label
    try:
        # Try to convert pos_label to int if it's a string representation of an integer
        if isinstance(pos_label, str) and pos_label.isdigit():
            converted_pos_label = int(pos_label)
        elif isinstance(pos_label, str):
            # For non-digit strings, keep as is and let sklearn handle it
            pass
    except (ValueError, TypeError):
        # If conversion fails, keep the original pos_label
        pass

    return df, converted_pos_label
...
def assert_pos_label_in_series(series, pos_label, default_pos_label='1'):
    ...
    Python 3.13 Upgrade: Enhanced to handle both original string labels
    and encoded numeric labels for compatibility with label encoding pipeline.
    ...
    """
    # Handle both string and numeric pos_label values by checking both
    # This accommodates cases where the series has been converted to integers for sklearn compatibility
    pos_label_candidates = [pos_label]
    try:
        # Try to convert pos_label to int if the series is numeric
        if series_type_is_numeric(series):
            pos_label_candidates.append(int(pos_label))
        # Also try float conversion for cases like '1.0'
        if isinstance(pos_label, str) and '.' in pos_label:
            pos_label_candidates.append(float(pos_label))
    except (ValueError, TypeError):
        pass

    # Additional candidates for common conversions
    try:
        # If pos_label is numeric string, add both int and float versions
        if isinstance(pos_label, str) and pos_label.replace('.', '').replace('-', '').isdigit():
            if '.' in pos_label:
                pos_label_candidates.append(float(pos_label))
                pos_label_candidates.append(int(float(pos_label)))
            else:
                pos_label_candidates.append(int(pos_label))
                pos_label_candidates.append(float(pos_label))
    except (ValueError, TypeError):
    ...
    # Check if any of the candidate values are in the series
    found = any(
        not series[series.isin([candidate])].empty for candidate in pos_label_candidates
    )

    # If still not found, check if the series contains any of the common encoded values
    if not found and series_type_is_numeric(series):
        # For numeric series, check if pos_label could be represented in the encoded values
        unique_values = series.unique()
        # If we have binary classification (0,1), be more lenient
        if len(unique_values) <= 2 and set(unique_values).issubset({0, 1, 0.0, 1.0}):
            if pos_label in ['1', '1.0', 1, 1.0]:
                found = 1 in unique_values or 1.0 in unique_values
            elif pos_label in ['0', '0.0', 0, 0.0]:
                found = 0 in unique_values or 0.0 in unique_values
...
def convert_df_to_categorical(df, warn_on_float_series=False):
    ...
    Returns:
        tuple: (df, numeric_conversions) where:
            df (pd.dataframe): input dataframe converted to categorical inplace
            numeric_conversions (dict): mapping of column names to their converted dtypes
    """
    numeric_conversions = {}
    ...
    # Convert entire df to string-type, but preserve numeric dtype for binary/multiclass data
    # that sklearn functions expect to be numeric
    result_df = df.copy()
    for j, (f, series) in enumerate(df.items()):
        unique_vals = set(series.unique())
        # Keep numeric dtype for binary or small multiclass data that sklearn expects as numeric
        if len(unique_vals) <= 10 and all(
            isinstance(val, (int, float, np.integer, np.floating))
            or (isinstance(val, str) and val.isdigit())
            for val in unique_vals
        ):
            # Keep as numeric for sklearn compatibility
            try:
                result_df.iloc[:, j] = pd.to_numeric(series)
                numeric_conversions[f] = 'numeric'
            except (ValueError, TypeError):
                # Fall back to string conversion if numeric conversion fails
                result_df.iloc[:, j] = series.astype(str)
                numeric_conversions[f] = 'string'
        else:
            result_df.iloc[:, j] = series.astype(str)
            numeric_conversions[f] = 'string'

    return result_df, numeric_conversions
...
def check_pos_label_and_average_against_data(
    ...
    for (f_a, s_a), (f_p, s_p) in zip(iter(actual_df.items()), iter(predicted_df.items())):
        # Check pos_label exists in actual field first (before cardinality check)
...
def prepare_clustering_scoring_data(df, label_field, feature_fields, mlspl_limits=None):
    ...
    label_df, _ = convert_df_to_categorical(df[label_field], warn_on_float_series=True)
    max_distinct_cat_values = int(mlspl_limits.get('max_distinct_cat_values_for_scoring', 100))

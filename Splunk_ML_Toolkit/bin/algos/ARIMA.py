...
class ARIMA(BaseAlgo):
    ...
    def _forecast(self, options, output_name=None):
        ...
        forecast_output, _, lower, upper = (
            self.estimator.get_forecast(**options).summary_frame(alpha=0.05).values.T
        )
        ...
        output[output_name] = forecast_output
    ...
    def fit(self, df, options=None):
        ...
        df[output_name].iloc[start : len(df) - self.holdback] = predicted_vals[start:]

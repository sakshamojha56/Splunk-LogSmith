...
class StateSpaceForecast(BaseAlgo):
    ...
    def compute_timestep(self, df):
        ...
        self._check_missing_rows(X[:effective_length], candidate)
    ...
    def _compute_effective_length(df, fields):
        ...
        Note that the effective end may be different from the end of the data frame df, because the specialdays column
    ...
    def generate_extra_time(self, df, effective_length, output, output_start):
        ...
        # Initialize with training data's last timestamp
        start_time = pd.Timestamp(self.datetime_information['last_timestamp'] * 1e9)

        # Try to use inference data's last timestamp (handles all date ranges)
        try:
            inference_time_values = self.convert_timefield_to_seconds(df, self.time_field)
            if effective_length <= len(inference_time_values):
                # Update to use the last timestamp from inference data
                inference_last_timestamp = np.max(inference_time_values[:effective_length])
                start_time = pd.Timestamp(inference_last_timestamp * 1e9)
        except (IndexError, KeyError, TypeError):
            # Keep fallback value if inference data access fails
            pass

        existing_time = df[self.time_field][output_start:effective_length]
        timestep = HumanTime.from_seconds(self.datetime_information['timestep'])
        freq = timestep.to_DateOffset()
        end_time = (
        ...
        # Generate timestamps starting from NEXT day after last inference data
        extra_time = pd.date_range(start=start_time + freq, end=end_time, freq=freq)

        # Calculate how many timestamps we need to add
        num_needed = max(0, steps - self.holdback)

        if self.time_field == '_time':
            try:
                extra_time = (extra_time.values.astype('int64') // 1e9)[:num_needed]
            except Exception as e:
                cexc.messages.error("'_time' column contains non-integer values")
                cexc.log_traceback()
                raise ValueError(e)
        else:
            extra_time = list(extra_time)[:num_needed]
    ...
    def _apply(self, df, out_params):
        ...
        if (
            ...
            self._check_missing_rows(time_values[:effective_length], timestep)
        ...
        # Preserve non-target columns (like holiday, time) before truncation
        non_target_cols = [col for col in df.columns if col not in self.time_series]
        preserved_fields = df[non_target_cols].copy() if non_target_cols else None

        # Remove spurious rows beyond effective_length from df (e.g., todays date)
        df = df.iloc[:effective_length].copy()
        ...
        # Restore non-target fields for all rows (including forecast rows)
        if preserved_fields is not None:
            for col in non_target_cols:
                if col in preserved_fields.columns:
                    # Align indices and fill in values from preserved_fields
                    if col not in df.columns:
                        df[col] = np.nan
                    # Use original values where they exist
                    df[col] = df[col].combine_first(preserved_fields[col])
